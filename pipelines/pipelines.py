...
from helpers.auxiliary_functions import (combine_centers, 
                                         parameters_extract,
                                         load_image_and_params)
from helpers.aux_plotter import plot_withcenters, plot_grayscale
from helpers.gaussian_estimation import (estimate_gaussians_centers, 
...
def ppl_lines_squares(image,
  ...
  if version == 'v1':
    ...
    if return_one == False:
      x_centers, y_centers = find_centers(x, y, z,
                                          params, version = 'v1',
                                          return_one = False)
...
def ppl_fit_gaussians(image,
  ...
  if means_init == 'laplace':
    ...
    amplitudes_apriori = image[2][center_coordinates[:, 0],
  ...
  (gmm,
   means,
   covariances) = estimate_parameters_gmm(nonzero_pairs[:, 0],
                                          nonzero_pairs[:, 1],
  ...
  for mean, covariance in zip(means, covariances):
      ...
      ellipses.append(patches.Ellipse(xy = mean,
                                      width = width,
                                      height=height,
                                      angle=angle,
                                      edgecolor = 'r',
  ...
  amplitudes = estimate_gaussians_amplitudes(means,
                                             image,
...
def ppl_label_YOLOv0(images,
  ...
  for n_image in range(images.shape[-1]):
    ...
    (amplitude,
     ...
     ys,
     dispersion_x,
     dispersion_y) = parameters_extract(images_features,
                                        n_image)
    ...
    labels, indices_out_of_image = create_labels(params,
                                                image_ranges,
                                                image_size,
                                                label_dir,
                                                f'gaussian_{n_image}',
...
def ppl_distribute_images_yolo(images_folder,
                               labels_folder,
                               yolo_folder,
                               range_images,
                               proportions,
    ...
    for i, n_image in enumerate(range_images):
        ...
        destination_path = os.path.join(yolo_folder, destination_folder,
                                        "images", f"gaussian_{n_image}.png")
        shutil.copy(source_path, destination_path)

        # Copy label
        source_path = os.path.join(labels_folder, f"gaussian_{n_image}.txt")
        destination_path = os.path.join(yolo_folder, destination_folder,
                                        "labels", f"gaussian_{n_image}.txt")
        shutil.copy(source_path, destination_path)

def ppl_yolo_predict_gaussians(mat_data,
                               n_image,
                               model,
                               image_path,
  ...
  amplitudes = estimate_gaussians_amplitudes(np.array(means),
                                             image,
                                             image_ranges = image_ranges,
                                             image_size = image_size)
  return means, covariances, amplitudes

def ppl_yolo_px_predict_gaussians(mat_data,
                                  n_image,
                                  model,
                                  image_path,
                                  image_ranges = [(0, 0.143), (0, 0.377)],
  ...
  (means,
  covariances,
  amplitudes_yolo) = ppl_yolo_predict_gaussians(mat_data,
                                                n_image,
                                                model,
                                                image_path,
                                                image_ranges = image_ranges,
  ...
  center_coordinates = np.array(list(zip(np.round(y_pixel),
                                         np.round(x_pixel))))

  # Compute new means refining the center (Center = Maximum amplitude)
  new_means = refine_centers(image[2],
                             center_coordinates,
                             step_trial_x = 1,
                             step_trial_y = 1,
  ...
  for n in range(len(new_means)):
    ...
    converted_new_means[n][1] = new_means[n][0] / (image_size[1] - 1) * image_ranges[1][-1]
    converted_new_means[n][0] = new_means[n][1] / (image_size[0] - 1) * image_ranges[0][-1]
  ...
  new_covariances) = estimate_parameters_PX(x,
                                            y,
                                            image[2],
                                            new_means,
                                            step_size = px_step_size)
  # To array
  new_covariances = np.array(new_covariances)

  # Infer overlapping
  if infer_overlapping == True:
    new_covariances = replace_covariances_outlier(new_covariances,
                                                  new_covariances_up,
                                                  new_covariances_down)

  # Compute new estimated amplitudes
  amplitudes_px = estimate_gaussians_amplitudes(new_means,
                                                image,
                                                image_ranges = image_ranges,
                                                image_size = image_size,
                                                center_in_pixel = True)
  return (converted_new_means, new_covariances, amplitudes_px, new_means,
          new_covariances_up, new_covariances_down)

def ppl_ensemble_predict_gaussians(n_image,
                                   mat_data,
                                   estimation_method,
                                   peak_method = None,
                                   image_path = None,
  ...
  """
  assert isinstance(estimation_method, list) & (len(estimation_method) > 1), "Unique method."
  ...
  for estimation in estimation_method:
      ...
      covariances,
      angles,
      estimated_amplitude) = ppl_select_estimation_method(
          image = image,
          n_image = n_image,
          mat_data = mat_data,
      ...
      generated_z,
      rmsep,
      error_gaussians) = compute_error_per_gaussian(image,
                                                    means,
  ...
  (all_gaussians_ensembled,
   id_gaussians_bettertwo) = ensemble_gaussians_estimation(list_all_gaussians[0],
  ...
  all_gaussians_ensembled = [(item[0],
                              item[1] * 2,
                              item[2]) for item in
                             all_gaussians_ensembled]
...
def ppl_select_estimation_method(**kwargs):
  ...
  if kwargs['estimation_method'] == 'gmm':
    ...
    covariances,
    angles,
    estimated_amplitude) = ppl_fit_gaussians(kwargs['image'],
                                means_init = 'laplace',
                                peak_method = 'laplace',
                                em_iterations = 1000,
    ...
    (means,
    covariances,
    estimated_amplitude) = ppl_yolo_predict_gaussians(kwargs['mat_data'],
                                                      kwargs['n_image'],
                                                      kwargs['yolo_model'],
                                                      image_path,
                                                      kwargs['image_ranges'],
    ...
    covariances,
    estimated_amplitude,
    *_) = ppl_yolo_px_predict_gaussians(kwargs['mat_data'],
                                       kwargs['n_image'],
                                       kwargs['yolo_model'],
                                       image_path,
                                       kwargs['image_ranges'],
...
def ppl_error_estimation(n_image,
                         mat_data,
                         estimation_method,
                         peak_method = None,
                         image_path = None,
  ...
  # Matches Gaussians
  row_ind, col_ind = match_gaussians(predicted_centers,
                                     true_centers)
  ...
  results = eval_identification(n_image,
                                means,
                                xs,
                                ys,
                                estimation_method,
  ...
  results.update(eval_centers(n_image,
                              predicted_centers,
                              true_centers,
                              estimation_method,
                              row_ind,
  ...
  results.update(eval_dispersion(n_image,
                                 covariances,
                                 dispersion_x,
                                 dispersion_y,
                                 estimation_method,
  ...
  results.update(eval_amplitude(n_image,
                                amplitude,
                                estimated_amplitude,
                                estimation_method,
                                row_ind,
  ...
  results.update(eval_error_per_gaussian(n_image,
                                         estimation_method,
                                         image,
                                         means,
