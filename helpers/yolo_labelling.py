...
def create_gaussian_mask(
    gaussian_params, img_ranges, img_size, n_contour_points=None, octagon_pol=False
):
    """
    ...
    x, y, z = generate_distribution(
        img_ranges[0], img_ranges[1], img_size[0], img_size[1], [gaussian_params]
    )
    ...
    if octagon_pol == False:
        ...
        if n_contour_points is not None:
            ...
            for contour in contours:
                ...
                norm_contours.append(
                    [
                        [p[1] / img_size[1], p[0] / img_size[0]]
                        for p in resampled_contour
                    ]
                )
        ...
        theta = np.linspace(
            0, 2 * np.pi, n_contour_points, endpoint=False
        ...
        # Normalize points
        norm_points = [
            [p[0] / img_size[1], p[1] / img_size[0]] for p in zip(x_points, y_points)
        ]
...
def create_labels(
    ...
    for i, gaussian_params in enumerate(gaussian_params_list):
        mask, norm_contours, *_ = create_gaussian_mask(
            gaussian_params,
            img_ranges,
            img_size,
            n_contour_points=None,
        ...
        width, height = (
            3 * gaussian_params[2] / img_ranges[0][-1],
        ...
        if task == "segment":
            ...
            label_line = (
                ...
                + " ".join(
                    [
                        f"{f'{cord[0]:.4f}'.rstrip('0')} {f'{cord[1]:.4f}'.rstrip('0')}"
                        for cord in norm_contours
                    ]
                )
            )
...
def out_of_image(gaussian_params, img_ranges):
    ...
    """
    return (
        (gaussian_params[0] <= img_ranges[0][0])
        | (gaussian_params[0] >= img_ranges[0][1])
