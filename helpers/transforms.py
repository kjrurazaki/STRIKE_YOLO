...
def gradient_transform(dx, dy, z):
    ...
    dz_dx_dx, dz_dx_dy, gradient_magnitude, gradient_direction = compute_gradient(
        dx, dy, dz_dx
    )
    # Second derivatives in y
    dz_dy_dx, dz_dy_dy, gradient_magnitude, gradient_direction = compute_gradient(
        dx, dy, dz_dy
    )
...
def laplace_treshold(x, y, z):
    ...
    nonzero_pairs = np.column_stack(
        (x_laplace[nonzero_indices], y_laplace[nonzero_indices])
    )
