...
from helpers.gaussian_estimation import (compute_mse, 
                                         sum_gaussians)
from helpers.yolo_labelling import out_of_image

def eval_identification(n_image,
                        means,
                        xs,
                        ys,
                        estimation_method,
  ...
  return {'Image' : [n_image],
          ...
          f'Percentage - {estimation_method}': [round(100 * number_estimated / number_gaussians_identifiable, 2)]}

def calculate_center_errors(predicted_centers,
                            true_centers,
                            match_row_ind,
                            match_col_ind,
    ...
    error_x = rmse_computation(matched_true_centers[:, 0],
                               matched_predicted_centers[:, 0],
                               return_percentage = return_percentage)

    error_y = rmse_computation(matched_true_centers[:, 1],
...
def eval_centers(n_image,
  ...
  (error_x,
   error_y,
   total_error) = calculate_center_errors(predicted_centers,
                                          true_centers,
                                          match_row_ind,
                                          match_col_ind,
                                          return_percentage)
...
def calculate_dispersion_errors(match_row_ind,
                                match_col_ind,
  ...
  error_x = rmse_computation(matched_true_covariance[:, 0, 0],
                             matched_predicted_covariance[:, 0, 0],
                             return_percentage = return_percentage)

  error_y = rmse_computation(matched_true_covariance[:, 1, 1],
  ...
  # Max relative errors
  max_x_error = np.max(np.abs((matched_true_covariance[:, 0, 0] -
                        matched_predicted_covariance[:, 0, 0])/
                        matched_true_covariance[:, 0, 0])) * 100

  max_y_error = np.max(np.abs((matched_true_covariance[:, 1, 1] -
                        matched_predicted_covariance[:, 1, 1])/
                        matched_true_covariance[:, 1, 1])) * 100

  total_max_error = np.sqrt(max_x_error ** 2 +
                            max_y_error ** 2)

  return error_x, error_y, total_error, max_x_error, max_y_error, total_max_error

def eval_dispersion(n_image,
                    covariances,
                    dispersion_x,
                    dispersion_y,
                    estimation_method,
  ...
  true_covariances = np.array([np.diag(mat) for
                               mat in list(zip(dispersion_x,
                                               dispersion_y))])
  ...
  (error_x,
   ...
   max_error_x,
   max_error_y,
   total_max_error) = calculate_dispersion_errors(match_row_ind,
                                              match_col_ind,
                                              covariances_wxy,
                                              true_covariances,
                                              return_percentage = return_percentage)
...
def eval_amplitude(n_image,
                   ...
                   return_percentage = False):
  # Error estimation
  error = rmse_computation(np.array(amplitude)[match_col_ind],
                           np.array(estimated_amplitude)[match_row_ind],
                           return_percentage = return_percentage)
  return {'Image' : [n_image],
          f'RMSE_amplitude - {estimation_method}' : [error]}

def match_gaussians(predicted_centers,
                    true_centers):
  """
...
def rmse_computation(true_values,
                     predicted_values,
                     return_percentage = False):
  """
  ...
  """
  error_abs = np.sqrt(mean_squared_error(true_values,
                                         predicted_values))
  if return_percentage:
    # Calculate percentage error for each sample
    error_percent =  np.mean(np.abs((true_values - predicted_values) /
                                    true_values)) * 100
...
def rmsep_each_gaussian(generated_image,
                        x,
                        y,
                        image,
                        image_ranges,
  ...
  for n_gaussian in range(len(all_gaussians)):
    ...
    if ((center[0] < image_ranges[0][0]) | (center[0] > image_ranges[0][1]) |
        (center[1] < image_ranges[1][0]) | (center[1] > image_ranges[1][1])):
      ...
      sub_region, cropped_region, _ = extract_region(x,
                                                     y,
                                                     generated_image,
                                                     center,
                                                     covariance)
      # Extract region - true one
      sub_region_truth, cropped_region_truth, _ = extract_region(x,
                                                                 y,
                                                                 image,
                                                                 center,
                                                                 covariance)
      # Append the error of the estimated Gaussian (RMSEP)
      error_gaussians.append(np.sqrt(compute_mse(cropped_region,
                                                 cropped_region_truth)))
  return error_gaussians

def compute_error_per_gaussian(image,
                               means,
                               covariances,
                               amplitudes,
                               image_ranges = [(0, 0.143),(0, 0.377)]
  ...
  all_gaussians = list(zip(means,
                          list(covariances),
                          amplitudes))

  # Generates gaussian
  generated_z = sum_gaussians(image[0], # x
                              image[1], # y
                              all_gaussians)

  # Error
  vec_rmsep = rmsep_each_gaussian(generated_z,
                                  image[0], # x
                                  image[1], # y
                                  image[2],
...
def eval_error_per_gaussian(n_image,
                            estimation_method,
                            image,
                            means,
                            covariances,
  ...
  (all_gaussians,
   generated_z,
   rmsep,
   vec_rmsep) = compute_error_per_gaussian(image,
                                           means,
                                           covariances,
                                           amplitudes,
                                           image_ranges = image_ranges)
  return {'Image' : [n_image],
          f'RMSEPixel - {estimation_method}': [rmsep]}
