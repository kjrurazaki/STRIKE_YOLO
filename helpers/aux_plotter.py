...
def plot_data_sav(line_z, smoothed, first_derivative, second_derivative, fig_size):
    ...
    axs[0].scatter(range(len(line_z)), line_z, label='Original data', color='blue', s=10)
    axs[0].plot(smoothed, label='Smoothed data', color='red')
    axs[0].plot(inflection_points, smoothed[inflection_points], 'go', label='Inflection points')
    axs[0].legend()
...
def gaussians_labelled(image_path,
                       label_path,
                       ax,
                       task = "segment",
                       colorbar = True,
  ...
  for i in range(len(labels)):
    if task == "segment":
      ...
      countours = [[float(x), float(y)] for x, y in zip(countours[::2], countours[1::2])]
      countours = np.array(countours)
      ax.plot(countours[:, 0] * grid_width, countours[:, 1] * grid_length, linewidth=1)
    ...
    rect = patches.Rectangle((x_min, y_min), box_width,
                             box_height, linewidth=1, edgecolor=edge_color,
                             facecolor='none')
    ax.add_patch(rect)
  # Plot the center points using scatter plot
  if plot_centers == True:
    ax.scatter([c[0] * grid_width for c in centers], [c[1] * grid_length for c in centers], s=1)
...
def plot_boxes_gmm(means,
  ...
  """
  for center, box in list(zip(means, [(sigmax, sigmay) for sigmax, sigmay in zip(np.array(covariances)[:, 0, 0], np.array(covariances)[:, 1, 1])])):
    # Create a Rectangle patch
    length = np.sqrt(np.abs(box[1])) * 1000 * beta
    width = np.sqrt(np.abs(box[0])) * 1000 * beta
    rect = patches.Rectangle((center[0] * 1000 - width/2, center[1] * 1000 - length/2), width, length, linewidth=1, edgecolor=edge_color, facecolor='none')

    # Add the patch to the Axes
    ax.add_patch(rect)

def plot_box_labels(image,
                    means,
                    covariances,
                    ax,
                    box_edge_color='r',
    ...
    (indice) in the image
    """
    plot_grayscale(image[0],
                   image[1],
                   image[2],
                   colorbar=colorbar,
                   ax=ax)

    for box in [(mean, covariance) for
                mean, covariance in zip(means, covariances)]:
        box_width = box[1][0] * np.sqrt(2)
        box_length = box[1][1] * np.sqrt(2)
        # Create a Rectangle patch
        rect = patches.Rectangle((box[0][0] - box_width/2,
                                  box[0][1] - box_length/2),
                                 box_width,
                                 box_length,
                                 linewidth=1,
...
def plot_3d_region(image,
    ...
    if show_surface:
      if colorscale is None:
          fig.add_trace(go.Surface(z=z_region,
                                  x=x_values,
                                  y=y_values,
    ...
    if show_points:
        ...
        hovertexts = None
        if annotate:
            hovertexts = [f"({i}, {j})" for
                          i, row in enumerate(mask) for
                          j, val in enumerate(row) if val]

        fig.add_trace(go.Scatter3d(x=x_values[mask],
                                   y=y_values[mask],
                                   z=z_region[mask],
                                   mode='markers',
...
def plot_2d_region(image,
    ...
    fig.add_trace(go.Heatmap(x=x_values[0, :],
                             y=y_values[:, 0],
                             z=z_region, colorscale=colorscale))

    if show_points:
        hovertexts = None
        if annotate:
            hovertexts = [f"({i}, {j})" for i, row in enumerate(mask) for j, val in enumerate(row) if val]

        fig.add_trace(go.Scatter(x=x_values[mask], y=y_values[mask],
    ...
    if extra_points is not None:
      for point in extra_points:
        fig.add_trace(go.Scatter(x=[point['x']],
                                 ...
                                 marker=dict(size = 5,
                                            color = point['color'])))
    fig.update_layout(title='2D Heatmap',
                      autosize=False,
                      width=800,
                      height=800,
                      margin=dict(t=65, b=40, l=60, r=60),
