...
def generate_gaussian_formula(
    x, y, x_centroid, y_centroid, sigma_x, sigma_y, amplitude
):
    return amplitude * np.exp(
        -(
            ((x - x_centroid) ** 2 / (2 * sigma_x**2))
            + ((y - y_centroid) ** 2 / (2 * sigma_y**2))
        )
    )


def generate_rotated_gaussian(
    x, y, x_centroid, y_centroid, sigma_x, sigma_y, amplitude, theta
):
    ...
    return amplitude * np.exp(
        -(
            ((x_rotated) ** 2 / (2 * sigma_x**2))
            + ((y_rotated) ** 2 / (2 * sigma_y**2))
        )
    )
...
def generate_distribution(
    x_range, y_range, num_subdivisions_x, num_subdivisions_y, params, rotation=True
):
...
def interpolate_line(start, end, steps, x, y, z, method="linear"):
    ...
    grid_z = griddata(
        (x.flatten(), y.flatten()), z.flatten(), (line_x, line_y), method=method
    )
...
def generate_noise(z, percentage):
    ...
    return np.random.uniform(
        -max_amplitude * percentage, max_amplitude * percentage, size=z.shape
    )
...
def plot_path(x, y, z, line_x, line_y, line_z):
    ...
    plt.figure()
    plt.plot(
        np.sqrt((line_x - line_x[0]) ** 2 + (line_y - line_y[0]) ** 2), line_z, "r-"
    )
    plt.xlabel("Distance")
    plt.ylabel("Z")
    plt.show()
...
def compare_z_values(x, y, z, line_x, line_y, line_z, z_true):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(
        np.sqrt((line_x - line_x[0]) ** 2 + (line_y - line_y[0]) ** 2),
        line_z,
        "r-",
        label="Interpolated",
    )
