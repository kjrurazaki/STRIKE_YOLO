...
def combine_centers(centers_x, centers_y):
    ...
    # Pairs of centers
    pairs = [(i, j) for sublist1, sublist2 in zip(centers_x, centers_y) for i in sublist1 for j in sublist2]
...
def save_all_gaussians(mat, folder):
  ...
  for save_id in range(0, mat['F_data'].shape[-1]):
    plt.imsave(fname=f"{folder}/gaussian_{save_id}.png", 
...
def load_image_and_params(n_image,
                          data_images,
                          data_parameters,
  ...
  """
  # Generate GRID
  x, y = generate_grid(image_ranges[0],
                      image_ranges[1],
                      image_size[0],
                      image_size[1])
  ...
  amplitude, xs, ys, dispersion_x, dispersion_y = parameters_extract(data_parameters, n_image)

  return image, amplitude, xs, ys, dispersion_x, dispersion_y

def convert_pixel_intensity(mat,
                            n_image,
                            image_path):
  """
...
def checkpoint_error(snapshot_folder_path,
                     error_dataframe,
                     annotation,
                     name):
  """
