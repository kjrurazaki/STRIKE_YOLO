...
def center_extraction(
    x,
    y,
    z,
    expected_x,
    expected_y,
    expected_xdeflection,
    expected_ydeflection,
    square,
    threshold,
    version,
    return_one=False,
    ...
    if version == "v0":
        ...
        if (len(max_indices_x) != 0) & (len(max_indices_y) != 0):
            if return_one == True:
                ...
                closest_index_x = find_closest_indices(
                    z_masked.shape, max_indices_x, axis=1
                )
                closest_index_y = find_closest_indices(
                    z_masked.shape, max_indices_y, axis=0
                )
...
def find_centers(x, y, z, params, version="v0", return_one=False):
    ...
    for expected_x in params["expected_xs"]:
        for expected_y in params["expected_ys"]:
            x_center, y_center = center_extraction(
                x,
                y,
                z,
                expected_x,
                expected_y,
                params["expected_xdeflection"],
                params["expected_ydeflection"],
                params["square"],
                params["threshold"],
                version=version,
