...
def generate_gaussian(x, y, mean, cov, amplitude):
    ...
    gauss = (
        amplitude
        * stats.multivariate_normal(mean, cov).pdf(np.dstack((x, y)))
...
@timer_decorator
def estimate_parameters_gmm(
    x, y, n_gaussians, means_init, em_iterations, covariance_gaussians_type
):
    """
...
def estimate_gaussians_centers(image, peak_method="laplace"):
    """
...
def estimate_gaussians_amplitudes(
    means,
    image,
    ...
    if center_in_pixel:
        ...
        center_location_pixel = [(int(x), int(y)) for x, y in zip(xs_center, ys_center)]
    # Amplitude estimation - in the intensity matrix, y is line and x column
    amplitudes = [image[2][y, x] for x, y in center_location_pixel]
    return amplitudes


def extract_center_dispersion(tensor_box, img_ranges, img_size):
    """
    NOT BEING USED
...
def correct_coordinates(center, image_shape, step_size):
    """
...
def center_and_neighbors(coord, center, step_size, direction):
    """
...
def construct_covariance_and_logical(n_gaussian, wxs, wys):
    """
    ...
    covariance = np.array(
        [
            [np.sign(wxs[n_gaussian]) * wxs[n_gaussian] ** 2, 0],
...
def estimate_parameters_PX(x, y, image, means_init, step_size):
    """
    ...
    for center in means_init:
        center = correct_coordinates(center, image.shape, step_size)
        # Reference coordinates
        x_center, x_up, x_down = center_and_neighbors(x, center, step_size, "x")
        # y
        y_center, y_up, y_down = center_and_neighbors(y, center, step_size, "y")

        # Amplitudes (intensities)
        (amplitude_center, amplitude_x_up, amplitude_x_down) = center_and_neighbors(
            image, center, step_size, "x"
        )
        (amplitude_center, amplitude_y_up, amplitude_y_down) = center_and_neighbors(
            image, center, step_size, "y"
        )

        # FWHM
        list_wxups.append(
            inverse_px(x_up - x_center, amplitude_x_up / amplitude_center)
        )
        list_wxdowns.append(
            inverse_px(x_down - x_center, amplitude_x_down / amplitude_center)
        )
        list_wyups.append(
            inverse_px(y_up - y_center, amplitude_y_up / amplitude_center)
        )
        list_wydowns.append(
            inverse_px(y_down - y_center, amplitude_y_down / amplitude_center)
        )

    # Don't use negative values
    for n_gaussian in range(len(list_wxups)):
        # Covariances
        covariance_up, logical_up = construct_covariance_and_logical(
            n_gaussian, list_wxups, list_wyups
        )
        list_covariances_up.append(covariance_up)

        (covariance_down, logical_down) = construct_covariance_and_logical(
            n_gaussian, list_wxdowns, list_wydowns
        )
        list_covariances_down.append(covariance_down)

        # Logical - eliminates the negative values to compute final covariance
        covariance = np.multiply(covariance_up, logical_up) + np.multiply(
            covariance_down, logical_down
        )
...
def refine_centers(
    image, center_coordinates, step_trial_x, step_trial_y, warning_boundary=True
):
    """
    ...
    for i, center in enumerate(center_coordinates):
        ...
        # Get region
        max_region = image[
            center[0] - step_trial_y : center[0] + step_trial_y + 1,
            center[1] - step_trial_x : center[1] + step_trial_x + 1,
        ]

        indices_max = np.unravel_index(np.argmax(max_region), max_region.shape)
        desloc_center = (indices_max[0] - step_trial_y, indices_max[1] - step_trial_x)
        new_center = center + desloc_center
        new_center_coordinates.append(new_center)

        if warning_boundary:
            if (desloc_center[0] == step_trial_y) | (desloc_center[1] == step_trial_x):
                print(
                    f"[Warning] Center {i}: Max in the boundary of region, might be outside."
                )
...
def covariances_index_replacemenvalue(**kwargs):
    ...
    replace_index = [
        idx
        for idx in np.multiply(
            np.array([not idx for idx in kwargs["diffs_onmargin"]]),
            range(len(kwargs["new_covariances_up"])),
        )
        if idx != 0
    ]
    ...
    replace_result = [
        ...
        for (i, j) in zip(up_values, down_values)
        if any(
            val >= 0
            for val in [
                i[kwargs["direction"]][kwargs["direction"]],
...
def replace_covariances_outlier(
    ...
    diffs_cov_x = [
        i[0][0] - j[0][0] for (i, j) in zip(new_covariances_up, new_covariances_down)
    ]
    diffs_cov_y = [
        i[1][1] - j[1][1] for (i, j) in zip(new_covariances_up, new_covariances_down)
    ]
    ...
    (replace_x_index, replace_x_result) = covariances_index_replacemenvalue(
        new_covariances_up=new_covariances_up,
        new_covariances_down=new_covariances_down,
        diffs_onmargin=diffs_x_onmargin,
        direction=0,
    )
    # Return indexes and values to replace
    (replace_y_index, replace_y_result) = covariances_index_replacemenvalue(
        new_covariances_up=new_covariances_up,
        new_covariances_down=new_covariances_down,
...
def ensemble_gaussians_estimation(
    all_gaussians_modelone,
    ...
    compare_errors = [
        x < y for x, y in zip(error_gaussians_modeltwo, error_gaussians_modelone)
    ]
    # Id better gaussians in modeltwo
    id_gaussians_bettertwo = [
        idx for idx, condition in enumerate(compare_errors) if condition
    ]
    ...
    all_gaussians_ensembled = replace_values_at_positions(
        all_gaussians_modelone.copy(), all_gaussians_modeltwo, id_gaussians_bettertwo
    )
